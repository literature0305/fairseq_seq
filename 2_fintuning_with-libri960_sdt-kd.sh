...
combined_data_dir=/home/Workspace/fairseq/data/combined_data_960_filtered
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    echo "stage 4: prepare finetuning"
    mkdir -p $combined_data_dir
    python 2_combine_960.py $combined_data_dir data/train-clean-100 data/train-clean-360 data/train-other-500

    cat $combined_data_dir/wav_dir.tsv > $combined_data_dir/train.original.tsv
    cat $combined_data_dir/labels.ltr > $combined_data_dir/train.original.ltr
    cat $combined_data_dir/labels.wrd > $combined_data_dir/train.original.wrd
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    python 2_filtering_libri960.py $combined_data_dir
fi
