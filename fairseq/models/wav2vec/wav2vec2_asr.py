...
class Wav2Vec2AsrConfig(FairseqDataclass):
    auxiliary_layer_for_kd_bert_base: bool = field(
        default=False, metadata={"help": "if true have auxiliary output for KD"}
    )
    w2v_path: str = field(
...
class Wav2VecCtc(BaseFairseqModel):
    ...
    def get_num_updates(self):
        return self.w2v_encoder.get_num_updates()
    ...
    # def forward_txt(self, **kwargs):
    #     return self.w2v_encoder.forward_txt(**kwargs)

    def forward_txt(self, mask, text):
        return self.w2v_encoder.forward_txt(0, mask, text)
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrConfig, output_size=None):
        ...
        if targ_d is not None:
            ...
            self.proj_kd = Linear(d, d)
            # self.proj_kd2 = Linear(d, d)
        ...
        # for knowledge distillation (or sequence discriminative training)
        self.num_head=4
        lm_feature_dim=768
        drop_out_in_positional_encoding=0.1
        self.embed = torch.nn.Sequential(
            torch.nn.Embedding(targ_d, d),
            PositionalEncoding(d, drop_out_in_positional_encoding),
        )
        self.multihead_attn1 = torch.nn.MultiheadAttention(d, self.num_head)
        self.multihead_attn2 = torch.nn.MultiheadAttention(d, self.num_head)
        self.multihead_attn3 = torch.nn.MultiheadAttention(d, self.num_head)
        self.linear_k1 = Linear(d, d)
        self.linear_q1 = Linear(d, d)
        self.linear_v1 = Linear(d, d)
        self.linear_k2 = Linear(d, d)
        self.linear_q2 = Linear(d, d)
        self.linear_v2 = Linear(d, d)
        self.linear_k3 = Linear(d, d)
        self.linear_q3 = Linear(d, d)
        self.linear_v3 = Linear(d, d)
        self.layer_norm1 = torch.nn.LayerNorm(d)
        self.layer_norm2 = torch.nn.LayerNorm(d)
        self.layer_norm3 = torch.nn.LayerNorm(d)
    ...
    def forward(self, source, padding_mask, target=None, **kwargs):
        ...
        if self.proj:
            x_kd = self.proj_kd(x)
            # x_kd2 = self.proj_kd2(x)
            x = self.proj(x)
            # x = torch.log_softmax(x * 0.6, dim=-1)
        else:
            x_kd = None
            # x_kd2 = None
        ...
        if target is not None:
            ...
            embed = embed.transpose(0,1) #.to(x.dtype)
            target_mask = (target == 1)

            residual = embed
            embed = self.layer_norm1(embed)
            embed, _ = self.multihead_attn1(self.linear_q1(embed),self.linear_k1(embed),self.linear_v1(embed), key_padding_mask=target_mask)
            embed = embed + residual
            residual = embed
            embed = self.layer_norm2(embed)
            embed, _ = self.multihead_attn2(self.linear_q2(embed),self.linear_k2(embed),self.linear_v2(embed), key_padding_mask=target_mask)
            embed = embed + residual
            residual = embed
            embed = self.layer_norm3(embed)
            embed, _ = self.multihead_attn3(self.linear_q3(embed),self.linear_k3(embed),self.linear_v3(embed), key_padding_mask=target_mask)
        else:
            embed = None
        ...
        return {
            ...
            "encoder_out_kd": x_kd,  # T x B x C
            "padding_mask": padding_mask,  # B x T,
            "layer_results": res["layer_results"],
        }  #"encoder_out_kd2": x_kd2,  # T x B x C

    def forward_txt(self, source, padding_mask, target=None, **kwargs):
        if target is not None:
            embed = self.embed(target)
            embed = embed.transpose(0,1) #.to(x.dtype)
            target_mask = (target == 1)
